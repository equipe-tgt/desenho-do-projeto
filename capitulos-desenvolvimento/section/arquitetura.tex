...
\gls{https}, que permite a aplicação
cliente realizar chamadas ao servidor através de \gls{url}, seja para
...
da lógica de um servidor, implementa \emph{listeners} para estas \glspl{url},
auxiliando a criação de pontos na aplicação do servidor focados na
comunicação com a aplicação cliente.

O uso do protocolo \gls{https} oferece algumas vantagens a aplicação
...
quando combinada ao modelo \gls{rest} na
construção da \gls{API}, o protocolo HTTP
